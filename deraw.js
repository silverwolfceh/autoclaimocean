...
//         WCzqz: d.sLveC
//       };
//       if (d.OCozb(d.qbduc, d.KZLOR)) {
//         if (Y) {
//           const U = U.apply(x, arguments);
//           t = null;
//           return U;
//         }
//       } else {
//         const U = A ? function () {
//           if (H.ndTyg(H.WjNai, H.ToIPB)) {
//             if (i) {
//               if (H.ndTyg(H.FhUlI, H.WCzqz)) {
//                 const x = i.apply(Y, arguments);
//                 i = null;
//                 return x;
//               } else {
//                 const a = A.apply(Y, arguments);
//                 i = null;
//                 return a;
//               }
//             }
//           } else {
//             throw new d(A.effects.status.error);
//           }
//         } : function () {};
//         A = false;
//         return U;
...
//       const Y = R("init");
//       if (!d.test(Y + "chain") || !A.test(Y + "input")) {
//         Y("0");
//       } else {
//         R();
//       }
...
//   })();
  ...
  const getIp = () => new Promise(async (l, y) => {
    const d = {
      rPgMf: function (A, Y) {
        return A(Y);
      },
    ...
    try {
      if (d.gvUmF(d.MTHve, d.KsEPH)) {
        d.rPgMf(d, A);
      } else {
        const Y = await k.get(d.bkjWd);
        d.rPgMf(l, Y.data);
      }
    } catch (i) {
      if (d.UqmyN(d.XrOBg, d.XrOBg)) {
        d.SHuYL(Y, i.whiteBright.bold(d.VjkMX) + " " + H.redBright.bold(r.message));
      } else {
        d.rPgMf(y, i);
      }
    }
  ...
  const sendOcean = async (y, d) => {
    ...
    A.ReJdt(c, "API-KEY Credit : " + Y.credit);
  ...
  const chainInfo = () => new Promise(async (A, Y) => {
    ...
    //   const U = await k.get("https://skripcode.com/sui-wave/apiv2.php", r);
  ...
  (async () => {
    try {
      ...
      console.log("=== " + O.cyanBright.bold.italic("https://github.com/downygg/OceanClaimer"));
      ...
      switch (i) {
        ...
        case "autotfocean":
          ...
          c(O.cyanBright.bold("Are your sure? (if you sure, just wait until 10 seconds.)"));
          await V(10000);
          var Y = parseInt(0);
